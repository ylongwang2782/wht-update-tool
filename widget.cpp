...
Widget::Widget(QWidget *parent)
    ...
    connect(ymodemFileTransmit, &YmodemFileTransmit::transmitProgress, this,
            &Widget::transmitProgress);
    connect(ymodemFileTransmit, &YmodemFileTransmit::transmitStatus, this, &Widget::transmitStatus);
    connect(bootloaderWaitTimer, &QTimer::timeout, this, &Widget::onWaitForBootloaderTimeout);
    connect(serialPort, &QSerialPort::readyRead, this, &Widget::onSerialDataReceived);
...
void Widget::on_comButton_clicked() {
    ...
    if (button_status == false) {
        ...
        if (serialPort->open(QSerialPort::ReadWrite) == true) {
            button_status = true;
            appendLog(QString("串口打开成功: %1 - %2")
                          .arg(ui->comPort->currentText(), ui->comBaudRate->currentText()));
            ...
            QMessageBox::warning(this, u8"串口打开失败", u8"请检查串口是否已被占用！",
                                 QMessageBox::Close);
        }
    } else {
        button_status = false;
...
void Widget::on_transmitBrowse_clicked() {
    QString fileName =
        QFileDialog::getOpenFileName(this, u8"选择固件文件", ".", u8"固件文件 (*.bin *.hex *.*)");
...
void Widget::transmitProgress(int progress) { ui->transmitProgress->setValue(progress); }
...
void Widget::sendUpgradeCommand() {
    ...
    if (!serialPort->isOpen()) {
        ...
        appendLog(QString("正在打开串口: %1, 波特率: %2")
                      .arg(ui->comPort->currentText())
                      .arg(ui->comBaudRate->currentText()));

        if (!serialPort->open(QSerialPort::ReadWrite)) {
            appendLog("错误: 无法打开串口！");
            QMessageBox::warning(this, u8"错误", u8"无法打开串口！", QMessageBox::Close);
            transmitButtonStatus = false;
            ui->comButton->setEnabled(true);
            ui->transmitBrowse->setEnabled(true);
            if (!ui->transmitPath->text().isEmpty()) {
...
void Widget::onWaitForBootloaderTimeout() {
    ...
    QMessageBox::warning(this, u8"超时", u8"等待MCU进入升级模式超时！请检查MCU连接和状态。",
                         QMessageBox::Close);
}
